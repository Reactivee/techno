...
class MaterialsController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($this->request->isPost) {
            if ($model->load($this->request->post())) {
                $img = $model->image = UploadedFile::getInstance($model, 'image');
                if ($img) {

                    $folder = Yii::getAlias('@frontend') . '/web/uploads/materials/';
                    ...
                    $img->saveAs($path);
                    $path = '/frontend/web/uploads/materials/' . $generateName . '.' . $img->extension;
                    $model->image = $path;
                }

                if ($model['oldAttributes']['image'] && !$img) {
    ...
    public function actionMaterialItem($id)
    {
